...
export const useSessionStore=defineStore('session',{
  ...
  actions:{
    async bootstrap(){telegram.ready();this.loading=true;try{const r=await api.getParsed('/api/session',sessionResponseSchema);this.user=r.user;this.stats=r.stats;this.config=r.config;this.authError='';this.publicOnly=false;await this.loadNotifications();}catch(e){if(e instanceof ApiError&&e.status===401){this.publicOnly=true;this.authError='Откройте V I B E через Telegram-бота, чтобы подавать заявки, создавать события и общаться.';}else this.authError=e instanceof Error?e.message:'Не удалось открыть сессию.';}finally{this.loading=false;}},
