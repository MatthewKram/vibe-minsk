...
export const useSessionStore=defineStore('session',{
  state:()=>({user:null as User|null,publicOnly:false,authError:'',loading:false,stats:{myEvents:0,pendingRequests:0,unreadNotifications:0},config:{appLinkBase:'',appUrl:'',mapEngine:'maplibre-osm'},notifications:[] as NotificationItem[],loadingNotifications:false}),
  ...
  actions:{
    ...
    async loadNotifications(){if(!this.user)return;this.loadingNotifications=true;try{const r=await api.getParsed('/api/notifications',notificationsResponseSchema);this.notifications=r.notifications;}finally{this.loadingNotifications=false;}},
