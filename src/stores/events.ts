...
export const useEventsStore=defineStore('events',{
  state:()=>({items:[] as EventItem[],my:[] as EventItem[],favorites:[] as EventItem[],current:null as EventItem|null,members:[] as EventMember[],loading:false,query:'',time:'Все',kind:'all'}),
  ...
  actions:{
    ...
    async loadMy(){const r=await api.getParsed('/api/me/events',eventsResponseSchema);this.my=r.events;},
    async loadFavorites(){const r=await api.getParsed('/api/favorites',eventsResponseSchema);this.favorites=r.events;},
    ...
    async uploadCover(eventId:string,file:File){const fd=new FormData();fd.append('file',file);await api.upload(`/api/events/${eventId}/cover`,fd);await Promise.all([this.loadOne(eventId),this.loadMy()]);}
  }
});
