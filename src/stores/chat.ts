...
export const useChatStore=defineStore('chat',{
  state:()=>({chats:[] as ChatItem[],messages:[] as MessageItem[],currentEventId:null as string|null,loadingChats:false,loadingMessages:false}),
  actions:{
    async loadChats(){this.loadingChats=true;try{const r=await api.getParsed('/api/chats',chatsResponseSchema);this.chats=r.chats;}finally{this.loadingChats=false;}},
    async loadMessages(eventId:string){this.loadingMessages=true;try{const r=await api.getParsed(`/api/chats/${eventId}/messages`,messagesResponseSchema);this.messages=r.messages;}finally{this.loadingMessages=false;}},
